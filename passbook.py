...
def get_frame(window):
    ...
    def bottom(radio_button_var):
        ...
        if radio_button_var == 1:
            ...
            ok_button.config(command=lambda: display(name=people_choices.item(people_choices.selection()[0])['values'][0]))#,
            ...
            ok_button.config(command=lambda: display(from_date=from_entry.get(), to_date=to_entry.get()))#, width=10, height=1)
            ok_button.grid(row=3, column=1)
            pre_frame = 3
